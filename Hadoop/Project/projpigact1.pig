-- Load data from HDFS
inputDia4 = LOAD 'hdfs:///user/divya/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDia5 = LOAD 'hdfs:///user/divya/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDia6 = LOAD 'hdfs:///user/divya/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
STORE namesOrdered INTO 'hdfs:///user/divya/outputs' USING PigStorage('|');
